        ...
        <div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
            <div class="space-y-8">
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-slate-200 p-6">
                    <h1 class="text-3xl font-bold tracking-tight text-gray-900">Edit Catatan Pelanggaran</h1>
                    <p class="mt-1 text-slate-600">Perbarui informasi pelanggaran.</p>
                </div>

                <!-- Form -->
                <div class="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
                    <div x-data="{
                        selectedKelas: '{{ old('kelas_id', $pelanggaran->santri->kelas_id) }}',
                        santriList: [],
                        isLoading: true,
                        fetchSantris() {
                        ...
                        <form action="{{ route('pelanggaran.update', $pelanggaran) }}" method="POST">
                            ...
                            <div class="p-6 space-y-6">
                                <div>
                                    ...
                                    <select id="kelas_id" x-model="selectedKelas" @change="fetchSantris()" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500" required>
                                        ...
                                        @foreach ($kelasList as $kelas)
                                            <option value="{{ $kelas->id }}" @selected(old('kelas_id', $pelanggaran->santri->kelas_id) == $kelas->id)>
                                                {{ $kelas->nama_kelas }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <select name="santri_id" id="santri_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500" :disabled="!selectedKelas || isLoading" required>
                                        ...
                                        <template x-for="santri in santriList" :key="santri.id">
                                            <option :value="santri.id" x-text="santri.nama" :selected="santri.id == {{ old('santri_id', $pelanggaran->santri_id) }}"></option>
                                        </template>
                                    </select>
                                ...
                                <div>
                                    ...
                                    <input id="jenis_pelanggaran" name="jenis_pelanggaran" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500" value="{{ old('jenis_pelanggaran', $pelanggaran->jenis_pelanggaran) }}" required />
                                </div>
                                <div>
                                    <label for="tanggal_kejadian" class="block text-sm font-medium text-gray-700">Tanggal Kejadian</label>
                                    <input id="tanggal_kejadian" name="tanggal_kejadian" type="date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500" value="{{ old('tanggal_kejadian', $pelanggaran->tanggal_kejadian->format('Y-m-d')) }}" required />
                                </div>
                                <div>
                                    <label for="keterangan" class="block text-sm font-medium text-gray-700">Keterangan (Opsional)</label>
                                    <textarea id="keterangan" name="keterangan" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500">{{ old('keterangan', $pelanggaran->keterangan) }}</textarea>
                                </div>
                                <div>
                                    <label for="dicatat_oleh" class="block text-sm font-medium text-gray-700">Dicatat Oleh</label>
                                    <input id="dicatat_oleh" name="dicatat_oleh" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500" value="{{ old('dicatat_oleh', $pelanggaran->dicatat_oleh) }}" required />
                                </div>
                            </div>
