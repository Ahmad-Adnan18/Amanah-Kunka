            ...
            <div class="space-y-8">
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-slate-200 p-6">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        ...
                        <a href="{{ route('pengajaran.kelas.index') }}" class="flex-shrink-0 w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-md bg-red-700 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-600">
                ...
                <!-- Grid Kartu Statistik Interaktif -->
                    ...
                    <a href="{{ route('admin.santri-management.index') }}" class="{{ $cardBaseClass }}">
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                {{-- [MODIFIKASI] Menambahkan detail jumlah putra & putri --}}
                                    ...
                                    <tbody class="bg-white divide-y divide-slate-200">
                                        @forelse ($perizinanAktif as $izin)
                                            <tr class="hover:bg-slate-50">
                                                <td class="px-6 py-4"><a href="{{ route('santri.profil.show', $izin->santri) }}" class="group"><div class="text-sm font-medium text-slate-900 group-hover:text-red-600">{{ $izin->santri->nama }}</div><div class="text-sm text-slate-500">{{ $izin->santri->kelas->nama_kelas ?? 'N/A' }}</div></a></td>
                                                <td class="px-6 py-4"><p class="text-sm font-medium text-slate-900">{{ $izin->jenis_izin }}</p><p class="text-sm text-slate-500 truncate">{{ $izin->keterangan }}</p></td>
                            ...
                            <table class="min-w-full divide-y divide-slate-200">
                                <tbody class="bg-white divide-y divide-slate-200">
                                    @forelse ($pelanggaranTerbaru as $pelanggaran)
                                        <tr class="hover:bg-slate-50">
                                            <td class="p-4"><a href="{{ route('santri.profil.show', $pelanggaran->santri) }}" class="group"><p class="font-medium text-slate-800 group-hover:text-red-600">{{ $pelanggaran->santri->nama }}</p><p class="text-sm text-slate-500">{{ $pelanggaran->jenis_pelanggaran }}</p></a></td>
                                        </tr>
                                    @empty
                                        <tr><td class="p-6 text-center text-slate-500">Tidak ada catatan pelanggaran baru.</td></tr>
                                    @endforelse
                                </tbody>
                            </table>
                        </div>
