                    ...
                    <form action="{{ route('admin.rooms.update', $room) }}" method="POST">
                        ...
                        <div class="p-6 space-y-6">
                            <div>
                                ...
                                <p class="mt-1 text-slate-600">Perbarui detail ruangan di bawah ini.</p>
                            </div>
                            <hr>
                            <div>
                                <label for="name" class="block text-sm font-medium text-gray-700">Nama Ruangan</label>
                                <input type="text" name="name" id="name" value="{{ old('name', $room->name) }}" required autofocus class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500">
                            ...
                            <div>
                                ...
                                <select name="type" id="type" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500">
                                    <option value="Biasa" {{ old('type', $room->type) == 'Biasa' ? 'selected' : '' }}>Biasa</option>
                                    <option value="Khusus" {{ old('type', $room->type) == 'Khusus' ? 'selected' : '' }}>Khusus (Lab, Aula, dll)</option>
                                </select>
